...
export function DashboardShell({ title, eyebrow, role, children, actions }: { title: string; eyebrow: string; role: 'member' | 'admin'; children: ReactNode; actions?: ReactNode }) {
  ...
  const friends = role === 'admin'
    ...
    : ['Bagas Mahpie', 'Sir Dandy', 'Jhon Tosan'];

  return (
    <div className="min-h-screen bg-[#b8bec6] px-4 py-6 text-[#111] md:px-8 lg:px-12 lg:py-12">
      <div className="mx-auto grid min-h-[calc(100vh-6rem)] max-w-[1710px] overflow-hidden rounded-[2rem] bg-[#f5f6fb] shadow-2xl shadow-slate-600/20 lg:grid-cols-[276px_1fr] xl:rounded-[2.5rem]">
        <aside className="flex flex-col gap-7 bg-white px-6 py-8 lg:px-11 lg:py-11">
          <a href="/" className="flex items-center gap-3 text-2xl font-semibold tracking-tight"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#6d5df6] text-white">✦</span>Coursue</a>
          ...
          <section className="hidden lg:block">
            ...
            <div className="grid gap-6">
              {friends.map((friend, index) => <div key={friend} className="flex items-center gap-3"><Avatar seed={index + (role === 'admin' ? 5 : 1)} /><div><p className="font-semibold">{friend}</p><p className="text-sm text-zinc-400">{role === 'admin' ? 'Team' : index === 1 ? 'Old Friend' : 'Friend'}</p></div></div>)}
            </div>
          </section>
          <div className="mt-auto flex gap-3 overflow-x-auto lg:block lg:space-y-4">
            ...
            <a className="flex min-w-max items-center gap-3 rounded-2xl px-3 py-3 text-lg font-medium" href="#settings"><Settings size={21} />Setting</a>
            <button id="logout" className="flex min-w-max items-center gap-3 rounded-2xl px-3 py-3 text-lg font-medium text-orange-500" type="button"><LogOut size={21} />Logout</button>
          </div>
        </aside>
        <main className="min-w-0 px-5 py-7 md:px-8 lg:px-9">
          <header className="grid gap-5 xl:grid-cols-[1fr_auto] xl:items-center">
            <div className="relative max-w-[920px]"><Search className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-400" size={22} /><input className="h-14 w-full rounded-full border border-zinc-200 bg-white px-14 text-lg outline-none placeholder:text-zinc-400" placeholder={role === 'admin' ? 'ค้นหาสมาชิกหรือคอร์ส...' : 'Search your course....'} /></div>
            <div className="flex items-center gap-4">{actions}<button className="grid h-14 w-14 place-items-center rounded-full border border-zinc-200 bg-[#f7f8fd]"><Inbox size={19} /></button><button className="grid h-14 w-14 place-items-center rounded-full border border-zinc-200 bg-[#f7f8fd]"><Bell size={19} /></button><span className="h-12 w-px bg-zinc-300" /><Avatar seed={role === 'admin' ? 9 : 4} /><div><p id="sidebar-name" className="text-lg font-medium">{role === 'admin' ? 'Admin' : 'Jason Ranti'}</p></div></div>
          </header>
